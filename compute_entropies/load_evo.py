...
from vortex.model.model import StripedHyena, AttentionBlock
...
class Evo2:
    ...
    def load_evo2_model(
        ...
        with torch.inference_mode():
            ...
            missing_in_state_dict = model_dict.keys() - state.keys()
            # Find keys that are in state_dict but not in model_dict
            extra_in_state_dict = state.keys() - model_dict.keys()
            ...
            filtered_dict = {k: v for k, v in state.items() if k in model_dict}
            ...
            if model.config.get("column_split", True):
                for layer_idx, block in enumerate(model.blocks):
                    if type(block) == AttentionBlock:
                        target_device = block.inner_mha_cls.Wqkv.weight.device

                        Wqkv = state[f"blocks.{layer_idx}.inner_mha_cls.Wqkv.weight"]
                        try:
                            bias = state[f"blocks.{layer_idx}.inner_mha_cls.Wqkv.bias"]
                        except:
                            bias = None
                        ...
                        if bias is not None:
                            ...
                            bias = bias.reshape(
                                block.num_attention_heads, 3, size_att_head
                            )
                            ...
                            try:
                                block.inner_mha_cls.Wqkv.bias.data = bias.to(
                                    target_device
                                )
                            except:
                                pass
