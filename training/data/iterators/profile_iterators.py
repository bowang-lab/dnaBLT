...
def vectorized_iterator_patch_lengths():
    ...
    arrow_iterator = iter(ArrowFileIterator(file_path=None, 
        file_format=file_format,
        worker_id=rank, 
        num_workers=world_size, 
        preprocess_dir=preprocess_dir,
    ...
    sequence_iterator = iter(SequenceIterator(
        preprocess_iterator=preprocess_iterator,
        max_seq_patches=args.seq_len * args.buffer_size
    ))

    start_time = time.time()
    batch = next(sequence_iterator)
    end_time = time.time()
    print(f"Time taken to get batch: {end_time - start_time} seconds")
