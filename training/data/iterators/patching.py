...
# --- Dummy implementations and constants ---
def get_local_rank():
    # In a real distributed setting, this returns the local GPU rank.
    return 0

def load_entropy_model(checkpoint_dir):
    # Dummy entropy model that returns random logits.
    class DummyEntropyModel(torch.nn.Module):
        def __init__(self):
            super().__init__()
        ...
        def forward(self, x):
            ...
            vocab = 260  # arbitrary vocab size for demonstration
            return torch.rand(batch, seq_len, vocab, device=x.device)
    return DummyEntropyModel()

BPE_ID = 1
OFFSET = 32

# --- Enum and PatcherArgs ---
...
# --- Helper functions ---
def entropy(scores):
    """Compute entropy per token from logits (using natural log)."""
    log_probs = F.log_softmax(scores, dim=-1)
    probs = torch.exp(log_probs)
    p_log_p = log_probs * probs
    return -p_log_p.sum(dim=-1)

def calculate_entropies(tokens: torch.Tensor, entropy_model, patching_batch_size, device: str | None = None, enable_grad: bool = False):
    """
    Splits the flattened tokens into chunks of size max_length and computes entropy for each chunk.
    Returns:
      - A tensor of entropies with shape [batch_size, seq_len]
      - The corresponding predictions reshaped as [batch_size, seq_len, vocab]
    """
    ...
    with grad_context:
        ...
        for split in splits:
            ...
            # Ensure token values are in expected range
        ...
        concat_preds = torch.cat(preds, dim=0).reshape(tokens.shape[0], tokens.shape[1], -1)
    return concat_entropies, concat_preds

def patch_start_mask_from_entropy_with_monotonicity(entropies, t):
    """Returns a boolean mask indicating patch start positions using token‐level entropy differences."""
...
def patch_start_mask_global_and_monotonicity(entropies, t, t_add=0):
    """Computes a patch start mask using a global threshold and a secondary threshold for differences."""
    bs, seq_len = entropies.shape
    if seq_len == 0:
        return entropies > t
    mask = torch.zeros_like(entropies, dtype=torch.bool)
...
def find_space_patch_start_ids(tokens):
    """
    Uses a space-like token heuristic (based on token values relative to OFFSET)
    to compute a boolean mask and then patch start IDs.
    """
    bs, seq_len = tokens.shape
    tokens_no_offset = tokens - OFFSET
    patch_end_mask = ((tokens_no_offset < ord("0")) |
                      ((ord("9") < tokens_no_offset) & (tokens_no_offset < ord("A"))) |
                      ((ord("Z") < tokens_no_offset) & (tokens_no_offset < ord("a"))) |
                      ((ord("z") < tokens_no_offset) & (tokens_no_offset < 0b10000000)) |
                      (0b11000000 <= tokens_no_offset))
    patch_end_mask[:, 1:] &= ~patch_end_mask[:, :-1]
    patch_end_mask |= tokens < OFFSET
    patch_start_mask = torch.cat(
        [torch.tensor([1, 1], device=tokens.device, dtype=torch.bool).unsqueeze(0).repeat(bs, 1),
         patch_end_mask[:, 1:]], dim=1)
    max_patches = patch_start_mask.sum(dim=1).max()
    patch_ids = torch.arange(seq_len + 1, device=tokens.device).unsqueeze(0).repeat(bs, 1)
    extra_patch_ids = torch.full((bs, seq_len + 1), seq_len + 1, dtype=torch.long, device=tokens.device)
    all_patch_ids = torch.cat((patch_ids, extra_patch_ids), dim=1)
    patch_start_mask_padded = torch.cat((patch_start_mask, ~patch_start_mask), dim=1)
    patch_start_ids = all_patch_ids[patch_start_mask_padded].reshape(bs, -1)[:, :max_patches]
    return patch_start_ids
...
def apply_bpe_patcher(tokens, bpe_patcher, patching_batch_size, device=None):
    """
    Applies a BPE-based patcher model to tokens (which must be on CPU).
    Returns a boolean mask indicating patch start positions.
    """
    assert tokens.device == torch.device("cpu"), f"{tokens.device} != cpu expects tokens to be on cpu"
    with torch.no_grad():
        bpe_patcher_device, device = to_device(bpe_patcher, device)
        bpe_patching_mask = []
        ...
        for split in splits:
            ...
            pred_cpu = pred[0].cpu()
            pred_cpu = pred_cpu.reshape(-1, pred_cpu.shape[-1])[: split.numel() - pad_size, :]
...
def find_bpe_patcher_patch_start_ids(tokens, bpe_patcher, patching_batch_size, device=None, include_next_token=True):
    bs, seq_len = tokens.shape
    first_ids = torch.tensor([0, 1], dtype=torch.long, device=tokens.device).unsqueeze(0).repeat(bs, 1)
    preds_truncation_len = first_ids.shape[1]
    token_input = tokens[:, 1:] if include_next_token else tokens[:, 1:-1]
    if token_input.shape[1] >= 1:
        patch_start_mask = apply_bpe_patcher(token_input, bpe_patcher, patching_batch_size, device)
        expected = tokens.shape[1] + include_next_token - preds_truncation_len
        assert patch_start_mask.shape[1] == expected, f"{patch_start_mask.shape[1]} != {expected}"
...
def find_lookup_table_start_mask(tokens: torch.Tensor, lookup_table: torch.Tensor, include_next_token=True):
...
def find_lookup_table_patch_start_ids(tokens: torch.Tensor, lookup_table: torch.Tensor, include_next_token=True):
    bs, seq_len = tokens.shape
    first_ids = torch.tensor([0, 1], dtype=torch.long, device=tokens.device).unsqueeze(0).repeat(bs, 1)
    preds_truncation_len = first_ids.shape[1]
    window_size = lookup_table.ndim
    assert window_size == 2, f"{window_size} != 2"
    token_input = tokens if include_next_token else tokens[:, : -preds_truncation_len + 1]
    if token_input.shape[1] >= window_size:
        patch_start_mask = find_lookup_table_start_mask(token_input, lookup_table, include_next_token)
        expected = tokens.shape[1] + include_next_token - preds_truncation_len
        assert patch_start_mask.shape[1] == expected, f"{patch_start_mask.shape[1]} != {expected}"
        patch_start_ids = patch_start_ids_from_patch_start_mask(patch_start_mask)
        patch_start_ids = torch.cat((first_ids, patch_start_ids + preds_truncation_len), dim=1)
...
class Patcher:
    def __init__(self, patcher_args: PatcherArgs):
        ...
        if self.realtime_patching:
            assert patcher_args.entropy_model_checkpoint_dir is not None, (
                "Cannot require realtime patching without an entropy model checkpoint"
            )
            entropy_model = load_entropy_model(patcher_args.entropy_model_checkpoint_dir)
    ...
    def patch(self, tokens: torch.Tensor, include_next_token: bool = False,
              preds: torch.Tensor | None = None, entropies: torch.Tensor | None = None,
              threshold: float = None) -> tuple[torch.Tensor, torch.Tensor | None]:
        """
        Computes patch lengths (and optionally returns computed scores) for a batch of token sequences.
        Supports several modes:
          - static: fixed patch size
          - byte: each token is its own patch
          - entropy: computes entropy per token and defines patches based on thresholds
          - bpe and bpe_patcher: use BPE-delim tokens or a BPE-based model
          - space: use space-like tokens to define patches
        """
        bs, seq_len = tokens.shape
        ...
        if self.patching_mode == PatchingModeEnum.static:
            patch_lengths = torch.zeros((bs, math.ceil(seq_len_next_tok / self.patch_size)),
                                        dtype=tokens.dtype, device=tokens.device).fill_(self.patch_size)
            ...
            patch_lengths = torch.ones((bs, seq_len_next_tok), dtype=tokens.dtype, device=tokens.device)
            ...
            if entropies is not None:
                ...
                scores, _ = calculate_entropies(tokens, self.entropy_model, self.patching_batch_size, self.device)
            if self.log_time:
                self.log["calculate_entropies"] += time.time() - s
                s = time.time()
            patch_start_ids = find_entropy_patch_start_ids(
                scores, self.patch_size, include_next_token=include_next_token,
                threshold=threshold if threshold is not None else self.threshold,
                threshold_add=self.threshold_add, monotonicity=self.monotonicity
            )
            if self.log_time:
            ...
            patch_start_ids = find_bpe_patcher_patch_start_ids(tokens, self.entropy_model,
                                                               self.patching_batch_size, self.device, include_next_token)
            patch_lengths = patch_lengths_from_start_ids(patch_start_ids, seq_len_next_tok)
